...
| Steel-type poison immunity | `637603374c1aa14b961f024ea0546e505d30f57d` | A | Backported in V2.1 | Added Steel alongside Poison in the modern BeforeMove status-immunity gate while preserving Corrosion; the older effect scripts already reject Steel later, but the early controller path must fail consistently | Poison/Steel/dual-type/Corrosion automated contract, clean build, and JIT boot PASS; in-battle fixture pending |
| Type Gems on status moves | `75ef1e1b39add295ea9b986cd91ecc5fc94f25ba` | A | Backported in V2.1 | The local BeforeMove Gem state matched only type and hit state, so a same-type status move could consume its Gem; activation now also requires a non-status split | Matching status/damaging/type/miss automated contract, clean build, and JIT boot PASS; in-battle fixture pending |
| Water Absorb on status moves | `cb721681e62012d864b01620a6d08034f08b948c` | A | Backported in V2.1 | Removed the obsolete nonzero-power requirement so modern Water status moves are intercepted while retaining the engine's blocked-context guard | Water/status/non-Water/blocked-context automated contract, clean build, and JIT boot PASS; in-battle fixture pending |
| Defiant / Competitive independent caps | `784e888295ce5cbac932771ce7524c28ad26fc10` | A | Backported semantically in V2.1 | Removed the shared Attack-stage precondition that incorrectly blocked Competitive at +6 Attack; retained the local pipeline's U-turn guard and each ability's own Attack or Special Attack cap | Defiant/Competitive boundary automated contract, clean build, and JIT boot PASS; in-battle fixture pending |
| DSpico-compatible antipiracy | `b15b01c9fa1add4ce196186ab1125f49b0ddc4fb` | A | Backported selectively in V2.1 | Replaced the legacy runtime decompression hook with direct card-check/dsprotect patches, restored the original ARM9 path, and decompressed overlay 123 at build time; unrelated toolchain cleanup was excluded | Static/generated-byte contract and clean build required; TWiLight/R4/DSpico hardware runs pending devices |
